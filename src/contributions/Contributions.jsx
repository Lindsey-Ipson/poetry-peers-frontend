...
function Contributions() {
  ...
  <div className="Contributions">
    ...
    <h2>You've found {uniqueThemeNames.size} themes in {uniquePoemIds.size} poems</h2>
    <h3>Your total tags: {tags.length}</h3>
    {tags.map((tag) => (

      <div className="card fade show d-block" tabIndex="-1" role="dialog" key={uuidv4()}>
        <div className="card-dialog card-xl" role="document">
          <div className="card-content">
            <div className="card-header">
              <h5 className="card-title">{tag.themeName} in &quot;{tag.poem.title}&quot; by {tag.poem.author}</h5>
            </div>
            ...
            <button type="button" className="btn btn-primary"
              ...
              <button type="button" className="btn btn-danger" data-dismiss="card" 
              onClick={() => handleDeleteTag(tag.themeName, tag.poemId, tag.highlightedLines)}>
                Delete tag
              </button>
            </div>
          </div>
